...
namespace Rox.Entertainment
{
    ...
    public class SteamUserData_v1
    {
            ...
            if (httpSteam)
            {
                ...
                switch (SteamUserData.ExtractSteamID(SteamID))
                {
                    ...
                    default:
                        ...
                        if (SteamUserData.ExtractSteamID(SteamID) != _Regex_Match_Not_Found_Any)
                            return await SendQueryMessage("steamid", SteamUserData.ExtractSteamID(SteamID),
                                new HttpClient(), IsMessageBox); //解析SteamID64
        ...
        private static async Task<SteamType> SendQueryMessage(string Type, string SteamID64, HttpClient httpClient,
            bool IsMessageBox)
        {
            try
            {
                ...
                switch ((int)response.StatusCode)
                {
                    ...
                    case 502: //服务器网关错误
                        ...
                        if (IsMessageBox)
                            MessageBox_I.Error(
                                $"上游服务错误, 在向 Steam 的官方 API 请求数据时遇到了问题, 这可能是他们的服务暂时中断，请稍后重试. 错误代码: {_Steam_Service_Error}",
                                _ERROR);
                    ...
                    case 401: //未经授权
                        ...
                        if (IsMessageBox)
                            MessageBox_I.Error(
                                $"认证失败。你提供的 Steam Web API Key 无效或已过期，或者你没有提供 Key。请检查你的 Key. 错误代码: {_Steam_Server_UnAuthenticated}",
                                _ERROR);
                        throw new IException.UAPI.Steam.UnAuthenticatedSteamKey();
                    case 500:
                        WriteLog.Error("API", "服务器内部错误, 请联系管理员, 错误代码: IException.UAPI.General.UnknowUAPIException");
                        if (IsMessageBox)
                            MessageBox_I.Error("服务器内部错误, 请联系管理员, 错误代码: IException.UAPI.General.UnknowUAPIException",
                                _ERROR);
            ...
            catch (Exception ex)
            {
                ...
                if (IsMessageBox)
                    MessageBox_I.Error(
                        $"获取 Steam 个人信息失败，请检查网络连接或API服务状态: {ex.Message}, 错误代码: {_Steam_Unknow_Exception}", _ERROR);
                return null;
            }
        }
        ...
        public class SteamType
        {
            ...
            /// 好友代码 （SteamID32）
            /// </summary>
            public string friendcode => GetFriendCode(steamid.ToString());

            /// <summary>
            /// SteamID3
            /// </summary>
            public string steamID3 => Converter.SteamID.ToSteamID3(steamid.ToString());
            ///// <summary>
            ///// Steam最后登出日期
            ///// </summary>
